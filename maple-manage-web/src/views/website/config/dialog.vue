        ...
        <el-row :gutter="35">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="参数键名" prop="configKey">
              <el-input :disabled="state.ruleForm.isSystem === true && state.dialog.type === 'edit'" v-model="state.ruleForm.configKey" placeholder="请输入参数键名" clearable></el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="是否内置" prop="isSystem">
              <el-switch v-model="state.ruleForm.isSystem" inline-prompt active-text="是" inactive-text="否"></el-switch>
            </el-form-item>
          </el-col>
...
const openDialog = (type: string, row) => {
  ...
  if (type === 'edit') {
    useWebConfig.getWebConfigById(row.id).then(res => {
      state.ruleForm = res;
      state.dialog.title = '修改网站配置';
      state.dialog.submitTxt = '修 改';
    });
  } else {
    state.dialog.title = '新增网站配置';
