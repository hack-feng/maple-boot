<template>
	<div class="system-dic-container layout-padding">
		<el-card shadow="hover" class="layout-padding-auto">
			<div class="system-dic-search mb15">
        <el-row :gutter="35">
          <el-form-item label="字典名称" class="ml10" size="default">
            <el-input v-model="state.tableData.param.query.dictName" placeholder="请输入字典名称"
          ...
          <el-button size="default" @click="resetQuery">
			...
			<el-table :data="state.tableData.records" v-loading="state.tableData.loading" style="width: 100%">
				...
				<el-table-column prop="dictCode" label="字段编码" show-overflow-tooltip></el-table-column>
...
const getTableData = () => {
  ...
  useDict.getPageList(state.tableData.param).then(res => {
...
const onRowDel = (row: RowDicType) => {
	ElMessageBox.confirm(`此操作将永久删除字典：“${row.dictName}”，是否继续?`, '提示', {
		...
		.then(() => {
      useDict.deleteDict(row.id).then(() => {
        getTableData();
